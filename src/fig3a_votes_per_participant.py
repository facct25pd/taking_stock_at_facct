...
votes_per_voter = votes_per_voter.sort_values("vote_count", ascending=True).reset_index(
    drop=True
)
...
# Plot as bar chart
ax.scatter(
    votes_per_voter["rank"],
    votes_per_voter["vote_count"],
    color=colors[3],
    s=100,
    label="n-votes",
    zorder=3,
...
ax.set_title(
    "Votes Cast per Participant (Ascending Order)", fontsize=25
)  # , fontweight='bold')
...
ax.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Median: {avg_votes:.1f} votes",
)
ax.legend(fontsize=15)
