...
participant_stats = df.groupby('voter-id').agg(
...
filtered_participants = participant_stats[participant_stats['duration_minutes'] > 30].copy()
...
print(filtered_participants[['voter-id', 'first_vote', 'last_vote', 'duration_minutes']].to_string())

# Sort participants by their first vote time for better visualization
filtered_participants_sorted = filtered_participants.sort_values('first_vote', ascending=False)
...
for date, label in zip(key_dates, date_labels):
    ...
    ax.text(date, ax.get_ylim()[1] * 1.02, label, ha='center', fontsize=17, color='dimgray')
...
ax.set_title('Participation Duration Timeline for Participants with >30 Minutes on Platform', fontsize=25, y=1.14)
