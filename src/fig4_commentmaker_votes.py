...
bars = ax.bar(
    range(len(participants)), n_comments, alpha=0.6, label="n-comments", color=colors[0]
)
...
ax.scatter(
    range(len(participants)), n_votes, color=colors[1], s=100, label="n-votes", zorder=3
)
...
ax.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label="Avg votes per participant (35.5)",
)  # {avg_votes_all:.1f}
...
ax.set_title(
    "Comments and Votes per Comment-Maker Participants (n-comments > 0)", fontsize=22
)
